...
CLASS STR64:
64 CONSTANT Maxlen   \ наибльшая длинна

   Maxlen   DEFS locVar
   1 DEFS lenVar

: len@ lenVar C@ ; \ получить длинну ( -- u )
: len! lenVar C! ; \ записать длинну ( u -- )

: STR@ locVar  len@ ; \  \ ( -- adr u )

: STR!  
	DUP   Maxlen  1 - >  IF DROP Maxlen  1 - THEN  \ ограничить длинну 
	DUP lenVar C!		 \ запомнить длинну
	locVar  SWAP CMOVE  \   \ скопировать строку
	0 locVar  lenVar C@  +   C!  \ пишем "0"  в конец
;
: S1  \ загрузить строку ~ac с очищением её
	DUP STR@ STR! STRFREE
;
...
: ERR-INCLUDE-PROBE
 ...
 ELSE 2DROP S" "  THEN  
  2DUP ?STR_FILE    IF 
  ['] INCLUDE-PROBE CATCH DROP
  
  IF CR ERR-STRING TYPE CR 1 ELSE 0 THEN   ELSE  1 THEN 
